...
public class LoggingFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        log.info("in LoggingFilter.filter method");
        ...
        ServerHttpRequestDecorator requestMutated = new ServerHttpRequestDecorator(exchange.getRequest()) {
            ...
            public Flux<DataBuffer> getBody() {
                RequestResponseLogger requestLogger = new RequestResponseLogger(getDelegate());
...
//                    }).doFinally((s) -> requestLogger.log());
        ...
        ServerHttpResponseDecorator responseMutated = new ServerHttpResponseDecorator(exchange.getResponse()) {
            ...
            public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                System.out.println("in writeWith()");
                RequestResponseLogger responseLogger = new RequestResponseLogger(getDelegate());
                if (LOGGABLE_CONTENT_TYPES.contains(String.valueOf(getHeaders().getContentType()).toLowerCase())) {
...
//                    });
    ...
    private static class RequestResponseLogger {
        private StringBuilder sb = new StringBuilder();

        RequestResponseLogger(ServerHttpResponse response) {
            sb.append("\n");
